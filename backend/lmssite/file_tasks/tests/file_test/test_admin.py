...
class CoursesTestsGuest(APITestCase):
    def setUp(self):
        ...
        self.teacher_has_access, self.teacher_profile = create_teacher("teacher")
        self.teacher_not_access, self.teacher_profile2 = create_teacher("teacher_not_access")

        teachers_id = [self.teacher_has_access.id]
        ...
        self.client.force_authenticate(user=self.teacher_has_access)
    ...
    def test_teacher_has_access(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(FileTasks.objects.count(), 1)
        self.assertEqual(FileTasks.objects.get().title, 'updated')

        # DELETE
        url = reverse('file_tasks-rud', kwargs={'pk': 1})
        response = self.client.delete(url, format='json')
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        self.assertEqual(FileTasks.objects.count(), 0)

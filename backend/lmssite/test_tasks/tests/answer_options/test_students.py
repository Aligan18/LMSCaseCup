...
class CoursesTestsGuest(APITestCase):
    ...
    def test_student_owner(self):
        ...
        self.assertEqual(FileTasksAnswer.objects.count(), 0)

    def test_student_not_owner(self):
        self.client.force_authenticate(user=self.student_not_owner)
        # GET All
        url = reverse('file_tasks_answer-list')
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
